...
def get_file(github, repo, ref, file):
    repo = github.get_repo(repo)
    file_content = cast(ContentFile, repo.get_contents(file, ref=ref))
...
def create_pr(
    ...
    new_rev: str,
    head: str,
    lock_contents: ContentFile,
    updated_lock_contents: str,
    pr_title,
    pr_body,
):
    pr_branch_name = f"refs/heads/update/{flake_input}-{new_rev}"
    try:
        repo.get_branch(pr_branch_name)
        print(
            f"[{flake_input}] Branch '{pr_branch_name}' already exist. Skipping update."
        )
        return
